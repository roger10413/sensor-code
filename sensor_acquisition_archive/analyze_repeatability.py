...
# 設定
# =========================================================
DATA_DIR      = "/home/aisc216/sensor_data/A121-20N-35cycle"   # ★★★ 資料夾路徑 (可用命令列參數覆蓋)
FILE_PATTERN  = "calib_*.csv"                  # ★★★ 只讀符合這個樣式的檔案
OUTPUT_BASE_DIR = "./analysis_output"          # ★★★ 輸出根目錄, 每次執行會在底下建一個時間戳子資料夾
DEFAULT_FULL_MODE = False                      # ★★★ True=預設就跑完整模式(含舊方法圖表),
...
# 統計
# =========================================================
def plot_astm_summary(astm, outdir):
    """
    畫各力值仿ASTM/ISO重複性的折線圖: X軸為力值, median主線連貫呈現趨勢,
    max用虛線疊加標示最壞情況上界。異常值(median超過門檻)用紅點標出,
    讓學長/教授一眼看出哪些力值需要特別說明。
    """
    import os
    import numpy as np
    import matplotlib.pyplot as plt

    table = astm["table"].dropna(subset=["non_repeatability_median_pct"])
    table = table.sort_values("force_bin_N")   # 依力值排序, 折線才有意義
    if len(table) == 0:
        return

    forces = table["force_bin_N"].values
    medians = table["non_repeatability_median_pct"].values
    maxs = table["non_repeatability_max_pct"].values

    # 判斷正常/異常: 用中位數的中位數當基準, 超過2倍标記異常
    # (這只是畫圖用的視覺門檻, 不是統計檢定, 報告時不要引用這條線本身)
    typical = np.median(medians)
    point_colors = ["indianred" if m > typical * 2 else "steelblue" for m in medians]

    fig, ax = plt.subplots(figsize=(10, 5))

    # max虛線(最壞情況上界) 先畫, 在底層
    ax.plot(forces, maxs, linestyle="--", color="gray", linewidth=1.2,
           marker="^", markersize=5, alpha=0.7, zorder=2,
           label="max (worst-case bound)")

    # median主線(典型值) 疊在上層, 逐點依異常與否上色
    ax.plot(forces, medians, linestyle="-", color="steelblue", linewidth=1.8,
           zorder=3, label="median (typical)")
    ax.scatter(forces, medians, c=point_colors, s=70, zorder=4,
              edgecolors="white", linewidths=1.2)

    for xi, m in zip(forces, medians):
        ax.annotate(f"{m:.2f}%", (xi, m), textcoords="offset points",
                   xytext=(0, 10), ha="center", fontsize=9)

    ax.set_xlabel("Force (N)")
    ax.set_ylabel("Non-repeatability / ISO 376 b' (%)")
    ax.set_title("Repeatability vs. force level (ASTM E74 / ISO 376 style)\n"
                 "median = typical value (cite this); dashed = max worst-case bound; "
                 "red dot = median > 2x overall typical")
    ax.grid(alpha=0.3, zorder=0)
    ax.legend(loc="upper right", fontsize=9)
    plt.tight_layout()

    path = os.path.join(outdir, "fig_astm_by_force.png")
...
def main():
    ...
    produced = [
        ...
        "fig_astm_by_force.png     各力值重複性長條圖 (新增)",
