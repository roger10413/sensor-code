...
# ★★★【收緊修正, 比照test13_v2.py】★★★ 舊設定[0.05,0.05,0.05,0.17,0.17,0.17]
# 允許橫向偏移50mm、旋轉將近10度, 是實測「整個會偏掉」的主因。
FM_LIM_XY_DEV   = 0.002      # X/Y非順應軸: 最大容許橫向偏差(m) = 2mm
FM_LIM_Z_SPEED  = 0.05       # Z順應軸: 最大下壓速度(m/s) = 50mm/s
FM_LIM_ROT_DEV  = 0.010      # Rx/Ry/Rz非順應軸: 最大容許旋轉偏差(rad) ≈ 0.57度
...
# --- CSV ---
CSV_DIR       = "/home/aisc216/sensor_data/A89F"
...
def send_force_mode_program(F_target, duration_s):
    ...
    ★★★【修正, 比照test13_v2.py】★★★ 舊版把tool_pose()直接寫在迴圈裡,
    等於每個控制週期(8ms)都重新計算一次參考座標系。工具一旦稍微偏一點,
    下個週期的參考座標系就跟著偏, 非順應軸的『程式設定位置』基準也跟著漂,
    偏移會逐週期累積放大——這是實測「整個會偏掉」的第二個原因(第一個是
    FM_LIMITS太鬆, 已在上面修正)。改成只在進force mode前擷取一次task_frame,
    整段保持期間沿用同一個參考座標系, 偏移不會滾雪球。
    另依官方手冊建議, 進force mode前插入至少0.02s的sleep, 避免順應軸方向
    殘留運動或高減速度影響力控啟動。
    """
...
def main():
    ...
    try:
        ...
        # =====================================================
        # ★★★【新增】pre-loop baseline ★★★
        # 循環1沒有『前一個循環的unload』可以借用當基準, 只能靠這裡補一段
        # 獨立的空載量測。循環2以後不需要重複做, 因為循環N結束時的unload
        # 本來就自然銜接循環N+1開始前的狀態(中間只隔著移動時間, 力值一路
        # 接近0), 不需要每個循環都額外插入。
        # 用cycle=0標記, 跟正式循環(1~N_CYCLES)的unload區分開來, 分析時
        # 可用 df[df.cycle==0] 篩出這段。
        #
        # ★ 修正: 這段之前必須先歸零FT300, 否則量到的是歸零前的原始
        # offset(實測約+20N, 不是真正的零力基準), 不能拿來跟其他已歸零
        # 過的baseline比較。這是第一次上機測試後才發現的問題, 已修正。
        # =====================================================
        print(f"\n  靜置 {SETTLE_TIME}s 後歸零FT300 (pre-loop baseline用)...")
        ...
        print(f"\n=== 循環開始前的pre-loop baseline (空載 {UNLOAD_TIME:.0f}秒) ===")
        read_and_log(UNLOAD_TIME, 0.0, "pre_loop_baseline", ft, daq,
                     csv_writer, csv_file, start_time, 0, plot_ctx,
                     ft_hist)
